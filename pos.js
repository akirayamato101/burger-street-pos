...
/* =================== IN-PAGE SECTION SIDEBAR =================== */

/**
 * Scrolls the .page-main container (not the window) to the given anchor.
 * Works for all pages that use the .page-with-sidebar layout.
 */
function scrollToSection(anchorId) {
  const anchor = document.getElementById(anchorId);
  if (!anchor) return;

  // .page-main is the scrollable container inside each page-with-sidebar
  const pageMain = anchor.closest('.page-main');
  if (pageMain) {
    const offset = anchor.offsetTop - 8;
    pageMain.scrollTo({ top: offset, behavior: 'smooth' });
  } else {
    // Fallback: scroll the page-content wrapper
    const pc = document.getElementById('pageContent');
    if (pc) pc.scrollTo({ top: anchor.offsetTop - 8, behavior: 'smooth' });
  }

  // Update active state immediately for responsiveness
  const prefix = anchorId.split('-anchor-')[0]; // e.g. "inv", "sum", "set"
  const attr = `data-ps-${prefix}`;
  document.querySelectorAll(`[${attr}]`).forEach(el => {
    el.classList.toggle('active', el.getAttribute(attr) === anchorId);
  });
}

/**
 * Scroll-spy: watches .page-main scroll and highlights the matching
 * sidebar item. Runs for whichever page-with-sidebar is currently active.
 */
(function setupScrollSpy() {
  // Map of page id prefix → sidebar attribute prefix
  const pageMap = {
    'page-inventory': 'inv',
    'page-summary': 'sum',
    'page-settings': 'set',
  ...
  function updateActiveSidebarItem(pageEl, prefix) {
    const pageMain = pageEl.querySelector('.page-main');
    if (!pageMain) return;

    const scrollTop = pageMain.scrollTop;
    const anchors = pageEl.querySelectorAll('.section-anchor[id]');
    let activeId = anchors[0] ? anchors[0].id : null;

    anchors.forEach(anchor => {
      if (anchor.offsetTop <= scrollTop + 60) {
        activeId = anchor.id;
      }
    });
    ...
    const attr = `data-ps-${prefix}`;
    document.querySelectorAll(`[${attr}]`).forEach(el => {
      el.classList.toggle('active', el.getAttribute(attr) === activeId);
    });
  }

  // Attach scroll listeners once DOM is ready
  document.addEventListener('DOMContentLoaded', () => {
    Object.entries(pageMap).forEach(([pageId, prefix]) => {
      const pageEl = document.getElementById(pageId);
      if (!pageEl) return;
      // Listen on the page-main scroller inside this page
      pageEl.addEventListener('scroll', () => {
        updateActiveSidebarItem(pageEl, prefix);
